...
+ (SVGCurve) readEllipticalArctoCommand:(NSScanner*)scanner path:(CGMutablePathRef)path relativeTo:(CGPoint)origin isRelative:(BOOL) isRelative
{
#if VERBOSE_PARSE_SVG_COMMAND_STRINGS
    SVGKitLogVerbose(@"Parsing command string: elliptical-arc-to command");
#endif
    NSString* cmd = nil;
    ...
    if( ! [scanner scanCharactersFromSet:cmdFormat intoString:&cmd] )
    {
        NSAssert( FALSE, @"failed to scan elliptical arc to command");
        return SVGCurveZero;
    }
    
    [SVGKPointsAndPathsParser readWhitespace:scanner];
    
    SVGCurve lastCurve = [SVGKPointsAndPathsParser readEllipticalArcArgumentSequences:scanner path:path relativeTo:origin isRelative:isRelative];
    return lastCurve;
}

+ (SVGCurve) readEllipticalArcArgumentSequences:(NSScanner*)scanner path:(CGMutablePathRef)path relativeTo:(CGPoint)origin isRelative:(BOOL) isRelative
{
    SVGCurve curve = [SVGKPointsAndPathsParser readEllipticalArcArguments:scanner path:path relativeTo:origin];
    
    while(![scanner isAtEnd])
    {
        curve = [SVGKPointsAndPathsParser readEllipticalArcArguments:scanner path:path relativeTo:(isRelative ? curve.p : origin)];
    }
